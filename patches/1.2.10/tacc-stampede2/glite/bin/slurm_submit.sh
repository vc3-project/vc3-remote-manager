...
then
  ...
  # Stampede2 doesn't allow memory requirements to be specified
...
# echo "module load tacc-singularity " >> $bls_tmp_file
echo "export PATH=/home1/05501/uscms/singularity-2.3.1/bin/:$PATH " >> $bls_tmp_file
...
#echo $jobID > ~/submit_output
#jobID=`echo $jobID | awk 'match($0,/[0-9]+/){print substr($0, RSTART, RLENGTH)}'`
#On Stampede2 job id is actually the last number in the string
jobID=`echo $jobID | awk 'match($0,/job [0-9]+/){print substr($0, RSTART, RLENGTH)}' | cut -f 2 -d' '`
if [ "X$jobID" == "X" ]; then
	rm -f $bls_tmp_file
...
#echo $blahp_jobID >> ~/blahp_job_ids
